...
router.get('/', (req, res, next) => {
  if(!req.user || req.user.username !== 'minet123') {
    res.redirect('/');
  } else {
...
router.get('/logs', (req, res, next) => {
  ...
  CryptXLogs.find().sort('-time').limit(150).exec(function(err, logs) {
...
router.get('/disqualify', (req, res, next) => {
  ...
  User.find({username:{$ne: 'minet123'}}).sort('username').exec(function(err, teams) {
